...
BEDROCK_VERSION=0.7.31-community
CODENAME=Poki
...
ifeq ($(ARCHITECTURE),x86_64)
    RELEASE_CFLAGS += -fcf-protection=full
endif
ifeq ($(ARCHITECTURE),i686)
    RELEASE_CFLAGS += -fcf-protection=full
endif
ifeq ($(ARCHITECTURE),aarch64)
    RELEASE_CFLAGS += -mbranch-protection=standard
endif
...
$(COMPLETED)/builddir:
	...
	mkdir -p $(SLASHBR)/libexec/plugins
	mkdir -p $(SLASHBR)/run
	mkdir -p $(SLASHBR)/share/brl-fetch/distros
	mkdir -p $(SLASHBR)/share/pmm/plugins
	mkdir -p $(SLASHBR)/etc/hooks/pre-init.d
	mkdir -p $(SLASHBR)/etc/hooks/post-enable.d
	mkdir -p $(SLASHBR)/etc/hooks/post-disable.d
	mkdir -p $(SLASHBR)/etc/hooks/pre-update.d
	mkdir -p $(SLASHBR)/etc/hooks/post-update.d
...
$(COMPLETED)/musl: vendor/musl/.success_fetching_source $(COMPLETED)/builddir $(COMPLETED)/linux_headers
	...
	# In addition to forcing static, we also use -isystem to advance the priority of Fortify
	if [ "$(ARCHITECTURE)" = "i386" ]; then \
		sed "s| -specs| -static -latomic -isystem $(SUPPORT)/fortify-include -specs|" $(SUPPORT)/bin/musl-gcc > $(SUPPORT)/bin/musl-gcc-new; \
	else \
		sed "s| -specs| -static -isystem $(SUPPORT)/fortify-include -specs|" $(SUPPORT)/bin/musl-gcc > $(SUPPORT)/bin/musl-gcc-new; \
	fi
...
vendor/zlib/.success_retrieving_source:
	rm -rf vendor/zlib/
	mkdir -p vendor/zlib
	cd vendor/zlib && wget -O- 'http://zlib.net/zlib-1.3.2.tar.gz' | gunzip | tar xf -
