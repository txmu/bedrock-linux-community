...
*   **Guix Shepherd Support:** Guix system initialization paths have been added to the default `bedrock.conf` so you can comfortably use Shepherd as your PID 1. However, according to the upstream, hijack process is not working, because of design assumptions of Guix. Neither did an attempt to use Guix binaries to fetch what's needed. More investigation is needed.
*   **Advanced Environment Isolation Control:** Building upon the Cross-Stratum Integration Engine, power users can now fine-tune path filtering without recompiling C code. You can explicitly whitelist required cross-stratum paths using `BRL_ENV_ALLOW`, forcibly block problematic directories with `BRL_ENV_DENY`, and define exactly which variables get filtered via `BRL_FILTER_ENVS`.
*   **Decentralized Plugin Managers:** To foster a community-driven tool ecosystem, we've bundled two distinct package managers for `brl` extensions themselves. **Sunlit Store** (`brl plugin`) fetches scripts from Tap registries and enforces mandatory code reviews before granting execution rights. **BRL Forge** (`brl forge`) takes a GNU Stow-like approach, securely fetching from direct URLs with SHA256 checks and using isolated symlinks to safely link/unlink experimental tools.
*   **Zero-Privilege Extensibility & Safe User Profiles:** You can now extend `pmm` to support user-local package managers (like `npm` or `pip`) simply by placing AWK definitions in `~/.config/bedrock/pmm/`—no root privileges required. Furthermore, we introduced `~/.bedrock_profile`. To prevent Arbitrary Code Execution, this is processed by a strict, non-eval key-value parser that safely merges only whitelisted variables (e.g., `PATH`, `BRL_ENV_ALLOW`) into your Bedrock session.
*   **Active Threat Defense & Auditing:** The brl warden plugin acts as a proactive security shield for your cross-stratum environment. It strictly locks down dynamic linker injection (LD_PRELOAD/LD_AUDIT), enforces YAMA ptrace restrictions to prevent memory tampering, secures ephemeral mounts (/tmp with nosuid/nodev), makes core routing binaries immutable, and actively sweeps user-spaces for spoofed xattr routing attributes.
